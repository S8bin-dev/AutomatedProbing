...
CORRECTION_FACTOR = 4.532    # For 6cm x 6cm sample
...
class AutomatedProbeSystem:
    ...
    def run_measurement(self):
        """Perform the four-point probe measurement."""
        print("\n" + "="*60)
        ...
        print(f"{'Set V':>8} | {'Meas V':>8} | {'Current (mA)':>12} | {'Rs (Ω/sq)':>10}")
        print("-" * 55)
        ...
        for set_v in voltages:
            try:
                ...
                results.append([meas_i, meas_v, abs(rs)])
                print(f"{set_v:>8.3f} | {meas_v:>8.3f} | {meas_i*1000:>12.3f} | {abs(rs):>10.2f}")
    ...
    def save_results(self, data):
        ...
        with open(f"{filename}.csv", 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Current (A)", "Voltage (V)", "Sheet Resistance (Ohm/sq)"])
            writer.writerows(data)
        print(f"✓ Saved data: {filename}.csv")
        
        # Calculate average Rs (filter outliers)
        ...
        plt.title(f"IV Curve - {self.sample_name}\nAvg Rs: {avg_rs:.2f} Ω/sq | Contact Height: {CONTACT_HEIGHT_MM}mm")
        ...
        print(f"\n✓ Average Sheet Resistance: {avg_rs:.2f} Ω/sq")
